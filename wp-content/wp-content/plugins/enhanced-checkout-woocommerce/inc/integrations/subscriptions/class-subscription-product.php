...
namespace GoDaddy\MWC\WordPress\HeadlessCheckout\Subscriptions;
...
class SubscriptionProduct {
    ...
    public function __construct() {
        add_action('graphql_register_types', array($this, 'register_types'), 10, 1);
    }
    ...
    public static function get_subscription_fields($fields = array()) {
        return array_merge(
            array(
                'signUpFee' => array(
                    'type'        => 'String',
                    'description' => __('Subscription pricing', 'gd-checkout'),
    ...
    private static function register_subscription_product_type() {
        register_graphql_object_type(
            'SubscriptionProduct',
            array(
                'description' => __('A subscription product object', 'gd-checkout'),
                            ...
                            'price'     => array(
                                ...
                                'args'        => array(
                                    'exclude' => array(
                                        ...
                                        'description' => __('Properties to be excluding from the price statement', 'gd-checkout'),
                                    ),
                                    'context'  => array(
                                        'type'        => 'DisplayContextEnum',
                                        'description' => __('How should price be format? Defaults to simple string', 'gd-checkout')
                                    ),
                                ),
                                'resolve'     => function ($source, array $args) {
                                    ...
                                    return !empty($price) ? html_entity_decode(get_woocommerce_currency_symbol()) . $price : null;
                                },
                            ),
                        )
                    ),
    ...
    private static function register_subscription_variable_product_type() {
        register_graphql_object_type(
            'SubscriptionVariableProduct',
            array(
                'description' => __('A subscription variable product object', 'gd-checkout'),
                'interfaces'  => Product_Types::get_product_interfaces(),
                'fields'      => array_merge(
                    Product::get_fields(),
                    Product_Types::get_pricing_and_tax_fields(
    ...
    private static function register_subscription_variation_type() {
        register_graphql_object_type(
            ...
            array(
                'description' => __('A subscription variable product variation object', 'gd-checkout'),
                'interfaces'  => array(
                    'Node',
